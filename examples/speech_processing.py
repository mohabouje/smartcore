...
class Processor:
    ...
    def __init__(self, buffer_size: int, fft_size: int, fs: int, winlen: float, winstep: float):
        ...
        self.fig = linear_chart.LinearChart()
        self.fig.initialize()
...
def main():
    ...
    processor = Processor(buffer_size=buffer_size, fs=args.fs, fft_size=1024, winlen=args.winlen, winstep=args.winstep)

    pa = PyAudio()
    stream = pa.open(format=paFloat32, channels=args.channels, frames_per_buffer=buffer_size,
                     rate=args.fs, input=True, stream_callback=processor.audio_processing)
