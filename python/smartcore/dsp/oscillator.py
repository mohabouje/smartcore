...
class Oscillator(Algorithm):
    ...
    def _configure(self):
        self._add_descriptor("sample_rate", "Sampling frequency in Hz.", float(), False)
        self._add_descriptor("frequency", "Oscillation frequency in Hz.", float(), False)
    ...
    def run(self, x):
        samples = (self.timestamp - self.__previous_timestamp) * self._value("sample_rate")
        t = np.linspace(self.__previous_timestamp, self.timestamp, samples, endpoint=False)
        self.__previous_timestamp = self.timestamp
        tp = self._value("type")
        if tp is "sawtooth":
            return signal.sawtooth(2 * np.pi * self._value("frequency") * t, width=self._value("width"))
        elif tp is "square":
            return signal.square(2 * np.pi * self._value("frequency") * t, duty=self._value("duty"))
        elif tp is "sinusoidal":
            return np.sin(2 * np.pi * self._value("frequency") * t)
        else:
